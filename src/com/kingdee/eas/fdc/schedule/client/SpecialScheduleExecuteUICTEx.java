...
public class SpecialScheduleExecuteUICTEx extends SpecialScheduleExecuteUI{

	public SpecialScheduleExecuteUICTEx() throws Exception {
		super();
	}
		...
		exportXls.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
			    ...
			    for(int i = 0; i < taskColl.size(); i++) {
			    	...
			    	rowAdd.getCell(1).setValue(TaskName);
			    	rowAdd.getCell(2).setValue(NatureTimes);
			    	rowAdd.getCell(3).setValue(planStart);
			    	rowAdd.getCell(4).setValue(planEnd);
			    	rowAdd.getCell(5).setValue(actualStartDate);
			    	rowAdd.getCell(6).setValue(actualEndDate);
			    	rowAdd.getCell(7).setValue(checkDate);
			    	rowAdd.getCell(8).setValue(AdminPerson);
			    	rowAdd.getCell(9).setValue(AdminDept);
			    }
				try {
	...
	private void initExportTable(KDTable table) {
		table.addHeadRow(0);
		String[] head = new String[] {"序号","任务名称", "任务工期", "计划开始日期", "计划完成日期",
