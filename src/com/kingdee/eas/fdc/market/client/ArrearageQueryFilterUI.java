...
public class ArrearageQueryFilterUI extends AbstractArrearageQueryFilterUI
{
	...
	public void clear() {
		...
		try {
			//(应收回款)默认选中		:   预收款(改成了预订金  by zhicheng_jin)、定金、楼款、首期、补差款、按揭款、公积金、退款
			...
			MoneyDefineCollection moneyDefineColl = MoneyDefineFactory.getRemoteInstance().getMoneyDefineCollection("select id ,name ,moneyType where sysType = '"+MoneySysTypeEnum.SALEHOUSESYS_VALUE+"' order by moneyType ");
	...
	public CustomerParams getCustomerParams() {
		...
		//(应收回款)		:   预收款(改成了 预订金  by zhicheng_jin)、定金、楼款、首期、补差款、按揭款、公积金、退款
		String resvMTypeStr = MoneyTypeEnum.PRECONCERTMONEY_VALUE +","+ MoneyTypeEnum.EARNESTMONEY_VALUE +","+ MoneyTypeEnum.HOUSEAMOUNT_VALUE +","
							   + MoneyTypeEnum.FISRTAMOUNT_VALUE +","+ MoneyTypeEnum.COMPENSATEAMOUNT_VALUE +"," + MoneyTypeEnum.LOANAMOUNT_VALUE +","+ MoneyTypeEnum.ACCFUNDAMOUNT_VALUE +","+ MoneyTypeEnum.REFUNDMENT_VALUE;		
		//MoenyType_Num = "moneyTypeNum";   //款项类型种类 ，存数字  0全部，1应收，2其它部分
	...
	public void setCustomerParams(CustomerParams cp) {
		...
		if(sellStatus!=null && sellStatus.length>0) {
			...
			for(int i=0;i<sellStatus.length;i++) sellStatusStr += sellStatus[i]+",";
			if(sellStatusStr.indexOf(RoomSellStateEnum.PREPURCHASE_VALUE)>=0){
				this.chkPrePur.setSelected(true);
			}
			if(sellStatusStr.indexOf(RoomSellStateEnum.PURCHASE_VALUE)>=0){
				this.chkPurchase.setSelected(true);
			}
			if(sellStatusStr.indexOf(RoomSellStateEnum.SIGN_VALUE)>=0){
				this.chkSign.setSelected(true);
			}
		}
