...
public class ContractChangeBillReportFacadeControllerBean extends AbstractContractChangeBillReportFacadeControllerBean
{
    ...
    protected RptParams _query(Context ctx, RptParams params, int from, int len) throws BOSException, EASBizException{
    	...
    	if(isClick!=null&&isClick){
    		...
    		sb.append(" select contract.fnumber contractNumber,contractType.flongNumber contractTypeNumber,contract.fmainContractNumber mainContractNumber,contract.fcontractPropert contractPropert,'' contractType,contract.fid id,ca.fid caId,cb.fid cbId,'' number,'' name,'' supplier,'' amount,ct.fname_l2 changeType,reason.fname_l2 reason,ca.fnumber caNumber,(case when cb.fid is not null then 1 else 0 end)copies,cb.fnumber cbNumber,cb.fname cbName,content.fchangeContent changeContent,cb.famount cbAmount,");
        	...
        	sb.append("	from T_CON_ChangeAuditBill ca left join T_FDC_ChangeType ct on ct.fid=ca.FAuditTypeID left join T_FDC_ChangeReason reason on reason.fid=ca.FChangeReasonID left join T_CON_ChangeSupplierEntry caEntry on ca.fid=caEntry.FParentID left join T_CON_SupplierContentEntry contentEntry on contentEntry.fparentid=caEntry.fid left join T_CON_ChangeAuditEntry content on content.fid=contentEntry.fcontentId left join t_con_contractBill contract on contract.fid=caEntry.FContractBillID");
    		...
    		sb.append(" select '' contractType,contract.fid id,ca.fid caId,cb.fid cbId,'' number,'' name,'' supplier,'' amount,ct.fname_l2 changeType,reason.fname_l2 reason,ca.fnumber caNumber,(case when cb.fid is not null then 1 else 0 end)copies,cb.fnumber cbNumber,cb.fname cbName,content.fchangeContent changeContent,cb.famount cbAmount,isnull(amount1.famount,0) amount1,isnull(amount2.famount,0) amount2,isnull(amount3.famount,0) amount3,isnull(amount4.famount,0) amount4,isnull(amount5.famount,0) amount5,isnull(amount6.famount,0) amount6,");
