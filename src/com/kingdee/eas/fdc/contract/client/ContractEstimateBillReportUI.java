...
import com.kingdee.eas.fdc.contract.ContractEstimateBillReportFacadeFactory;
 ...
 * output class name
 */
public class ContractEstimateBillReportUI extends AbstractContractEstimateBillReportUI
{
    private static final Logger logger = CoreUIObject.getLogger(ContractEstimateBillReportUI.class);
    
    private boolean isQuery=false;
    private boolean isOnLoad=false;
    public ContractEstimateBillReportUI() throws Exception
    {
        super();
        tblMain.checkParsed();
	...
	protected ICommRptBase getRemoteInstance() throws BOSException {
		return ContractEstimateBillReportFacadeFactory.getRemoteInstance();
	}
	...
	protected void query() {
		...
		CRMClientHelper.changeTableNumberFormat(tblMain, new String[]{"contractAmount","amount"});
		FDCHelper.formatTableDate(tblMain, "changeDate");
		
		tblMain.getColumn("ecNumber").getStyleAttributes().setFontColor(Color.BLUE);
		ContractBillReportUI.getFootRow(tblMain, new String[]{"contractAmount","amount"});
	}
        	         ...
        	         for (int sort = 0;sort < key.length; sort++) { 
        	        	 String contractType = (String) key[sort];
         	        	 ...
         	        	 for(int i=0;i<list.size();i++){
         	        		 ...
         	        		 contractAmount=contractAmount.add(row.getCell("contractAmount").getValue()==null?FDCHelper.ZERO:(BigDecimal)row.getCell("contractAmount").getValue());
         	        	 ...
         	        	 addRow.getCell("contractAmount").setValue(contractAmount);
	...
	protected void tblMain_tableClicked(KDTMouseEvent e) throws Exception {
		if (e.getType() == KDTStyleConstants.BODY_ROW && e.getButton() == MouseEvent.BUTTON1 && e.getClickCount() == 2) {
			if(tblMain.getColumnKey(e.getColIndex()).equals("ecNumber")){
				String id=(String)this.tblMain.getRow(e.getRowIndex()).getCell("id").getValue();
				if(id!=null){
					UIContext uiContext = new UIContext(this);
					uiContext.put(UIContext.OWNER, this);
					uiContext.put(UIContext.ID, id);
					IUIWindow uiWindow = UIFactory.createUIFactory(UIFactoryName.NEWTAB).create(ContractEstimateChangeEditUI.class.getName(), uiContext, null, OprtState.VIEW);
					uiWindow.show();
				}
			}
		}
