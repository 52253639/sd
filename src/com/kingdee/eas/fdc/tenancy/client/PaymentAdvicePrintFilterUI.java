 ...
 *
 */
public class PaymentAdvicePrintFilterUI extends AbstractPaymentAdvicePrintFilterUI
{
    private static final Logger logger = CoreUIObject.getLogger(PaymentAdvicePrintFilterUI.class);
	...
	protected void isLack_mouseClicked(MouseEvent e) throws Exception {
		super.isLack_mouseClicked(e);
		if(isLack.isSelected()){
			lackDates.setVisible(true);
//			contAppDate.setEnabled(false);
//			contAppDate2.setEnabled(false);
		}else{
			lackDates.setVisible(false);
//			contAppDate.setEnabled(true);
//			contAppDate2.setEnabled(true);
		}
	}
	public PaymentAdvicePrintFilterUI() throws Exception
    {
        super();
    }
    public PaymentAdvicePrintFilterUI(ListUI listUI, ItemAction actionListOnLoad)
	throws Exception {
    	super();
    	this.listUI = listUI;
...
//		//可以多选
     ...
     * @throws BOSException
     */
//    private void initAreaComment(KDTextArea box) throws EASBizException, BOSException{
//    	
//    	box.setMaxLength(255);
//		IRowSet rowSet=PaymentAdvicePrintFacadeFactory.getRemoteInstance().getComment();
//		try {
//			while(rowSet.next()){
//				String str=rowSet.getString("comment");
...
//			String sql="update T_TEN_PaymentAdviceComment set fcomment ='"+this.comment.getText()+"' WHERE FNUMBER='1'";
...
//			}
   ...
   public CustomerParams getCustomerParams() {
	   ...
	   //add by hh
	   if(isLack.isSelected()){
		   param.add("isLack", isLack.isSelected());
	   }
	   if(kDNumberTextField1.getValue()!=null){
		   param.add("arrearageDay", kDNumberTextField1.getIntegerValue());
	   }
...
//		}
		...
		//是否
			...
			//add by hh
			this.isLack.setSelected(para.getBoolean("isLack"));
			this.kDNumberTextField1.setValue(para.getInteger("arrearageDay"));
   ...
   public FilterInfo getFilterInfo() {
	 ...
	 //合同name
	   ...
	   //by hh
		if(para.isNotNull("isLack")){
			filter.getFilterItems().add(new FilterItemInfo("isLack", Boolean.valueOf(para.getBoolean("isLack"))));
			  
//			Calendar calendarDate = Calendar.getInstance();
//			calendarDate.clear();
//			BigDecimal i=(BigDecimal)kDNumberTextField1.getValue();
//			calendarDate.setTime(new Date());
//			calendarDate.add(Calendar.DAY_OF_YEAR, -i.intValue());
//			filter.getFilterItems().add(new FilterItemInfo("lackDate",calendarDate.getTime(),CompareType.GREATER_EQUALS));
		}
		if(para.isNotNull("arrearageDay")){
			filter.getFilterItems().add(new FilterItemInfo("arrearageDay", para.getInteger("arrearageDay")));
		}
    	clear();
	   return filter;	
   }
...
//		   this.sellProject.setValue(null);
		   ...
		   kDNumberTextField1.setValue(null);
//		   init();
	} catch (Exception e) {
		this.handUIException(e);
	}
