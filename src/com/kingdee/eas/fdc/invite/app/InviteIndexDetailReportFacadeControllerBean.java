...
public class InviteIndexDetailReportFacadeControllerBean extends AbstractInviteIndexDetailReportFacadeControllerBean
{
    ...
    protected RptParams _createTempTable(Context ctx, RptParams params)    throws BOSException, EASBizException
	{
	    ...
	    header.setLabels(new Object[][]{ 
	    		{
	    			"pcId","supplierId","levelNumber","框架合约编码","框架合约名称","规划金额(元)","实际采购模式","已完成采购的合同总项数","新增供应商中标情况","新增供应商中标情况","新增供应商中标情况","目标成本单价达成情况","目标成本单价达成情况","目标成本单价达成情况","目标成本单价达成情况","招标率完成情况","招标率完成情况","招标率完成情况","招标率完成情况","招标率完成情况","采用最低价中标的合同数","采用最低价中标的合同数","采购计划达成情况"	,"采购计划达成情况"
	    		}
	    		,
	    		{
	    			"pcId","supplierId","levelNumber","框架合约编码","框架合约名称","规划金额(元)","实际采购模式","已完成采购的合同总项数","中标单位","新增供应商中标项数","比率","已完成采购的合同金额(元)","已完成采购的单方造价(元)","已完成采购且符合采购计划中目标成本单价的合同项数","比率","制度要求采购方式","实际采购方式","制度要求招标且实际采用招标的合同项数","制度要求招标的合同项数","比率","采用最低价中标的合同数（不包含直接委托）","比率","已按时完成采购的合同项数","比率"
				}
	    },true);
    ...
    protected RptParams _query(Context ctx, RptParams params, int from, int len) throws BOSException, EASBizException{
    	...
    	sb.append(" select invEntry.fprogrammingContractID pcId,con.supplierId,entry.flevel levelNumber,entry.flongNumber number,entry.fname name,entry.famount amount,purMode.fname_l2 purMode,isnull(con.account,0) value1,con.supplier supplier,isnull(newcon.account,0) value2,null rate1,isnull(con.amount,0) value3,(case when area.area is null or area.area=0 then 0 else isnull(con.amount,0)/area.area end) value4,price.account value5,null rate2,form.fname_l2 form,realForm.fname_l2 realForm,(case when form.fname_l2=realForm.fname_l2 and form.fname_l2='招标' then isnull(con.account,0) else 0 end) value6,(case when form.fname_l2='招标' then isnull(con.account,0) else 0 end) value7,null rate3,isnull(result.account,0) value8,null rate4,isnull(finish.account,0) value9,null rate5");
    	...
    	sb.append(" left join (select count(*) account,sum(con.famount) amount,supplier.fid supplierId,supplier.fname_l2 supplier,con.fprogrammingContract fprogrammingContractID from T_CON_ContractBill con left join t_bd_supplier supplier on supplier.fid=con.fpartBId where con.fstate='4AUDITTED' group by supplier.fname_l2,con.fprogrammingContract,supplier.fid) con on con.fprogrammingContractID=invEntry.fprogrammingContractID");
    	sb.append(" left join (select count(*) account,con.fprogrammingContract fprogrammingContractID from T_CON_ContractBill con where con.fstate='4AUDITTED'");
    	...
    	sb.append(" group by con.fprogrammingContract) newcon on newcon.fprogrammingContractID=invEntry.fprogrammingContractID");
