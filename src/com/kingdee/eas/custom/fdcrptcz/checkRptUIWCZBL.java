...
tblCheck.getStyleAttributes().setHided(true);
		...
		headRow.getCell("value1").setValue("制度要求招标且实际采用招标的合同项数");
		headRow.getCell("value2").setValue("制度要求招标的合同项数");
	...
	public void setValue(IRow row, String longNum){
		for( ; step<rowCount; step ++){
			if(tblCheck.getRow(step).getCell("longNumber").getValue() != null 
				...
				BigDecimal value1 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("column18").getValue()); //制度要求招标且实际采用招标的合同项数
				BigDecimal value2 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("column19").getValue()); // 制度要求招标的合同项数
