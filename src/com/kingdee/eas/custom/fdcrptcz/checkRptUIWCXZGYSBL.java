...
public class checkRptUIWCXZGYSBL  extends checkRptUI2 {

	public checkRptUIWCXZGYSBL() throws Exception {
		super();
...
tblCheck.getStyleAttributes().setHided(true);
		...
		headRow.getCell("value1").setValue("招标新增供应商中标项数");
		headRow.getCell("value2").setValue("已完成采购的招标的合同总项数");
			...
			switch (i){
				...
				case 4:
					...
					if(tblCheck.getRow(0) != null){
						...
						if(tblCheck.getRow(0).getCell("column17").getValue() != null	){
	...
	public void setValue(IRow row, String longNum){
		for( ; step<rowCount; step ++){
			if(tblCheck.getRow(step).getCell("longNumber").getValue() != null 
				...
				BigDecimal value1 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("winBuildCount").getValue()); //新增供应商中标项数
				BigDecimal value2 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("column17").getValue()); // 已完成采购的招标的合同总项数
				
				row.getCell("value1").setValue(value1 == null ?0:value1);
				row.getCell("value2").setValue(value2.intValue());
				
				if(tblCheck.getRow(step).getCell("column17").getValue() != null	){
					if(value2.equals(FDCHelper.ZERO) || value2.toString().equals("0.00") || value2.toString().equals("0")){
						row.getCell("value").setValue("0.00%");
					}else{
						row.getCell("value").setValue(value1.divide(value2,2,BigDecimal.ROUND_HALF_UP).multiply(FDCHelper.ONE_HUNDRED) +"%");
