...
public class checkRptUIWCMBCBDJDCL  extends checkRptUI2  {

	public checkRptUIWCMBCBDJDCL() throws Exception {
		super();
	...
	public void onLoad() throws Exception {
		...
		headRow.getCell("value1").setValue("已完成采购且符合采购计划中目标成本单价的合同项数");
	...
	public void setValue(IRow row, String longNum){
		for( ; step<rowCount; step ++){
			if(tblCheck.getRow(step).getCell("longNumber").getValue() != null 
				...
				BigDecimal value1 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("targetCostCount").getValue()); //新增供应商中标项数
				BigDecimal value2 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("totalCon").getValue()); // 已完成采购的招标的合同总项数
