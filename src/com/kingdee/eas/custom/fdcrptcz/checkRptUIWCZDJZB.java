...
// 最低价中标
public class checkRptUIWCZDJZB extends checkRptUI2{

	public checkRptUIWCZDJZB() throws Exception {
		super();
...
tblCheck.getStyleAttributes().setHided(true);
		...
		headRow.getCell("value1").setValue("采用最低价中标的合同项数");
	...
	public void setValue(IRow row, String longNum){
		for( ; step<rowCount; step ++){
			if(tblCheck.getRow(step).getCell("longNumber").getValue() != null 
				...
				BigDecimal value1 = FDCHelper.toBigDecimal(tblCheck.getRow(step).getCell("minConCount").getValue()); //采用最低价中标的合同项数
